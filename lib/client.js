  ...
  env.getTarget(options.target, function (err, target) {
    Doc.get(id, target, function (err, res, body) {
      var doc = JSON.parse(body)
        ...
        , file = options.file || path.join(env.root, 'data', options.target, doc.type, doc._id + '.json');
...
Client.prototype.draft = function(file, options) {
  ...
  if (!options.file) {
    var subdir = options.model || env.config.defaults.model
      , target = options.target || env.config.defaults.target;
    if (path.dirname(file) == '.') file = path.join('data', target, subdir, file);
