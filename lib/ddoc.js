...
var DDoc = Doc.extend({
  ...
  load: function (dir, options) {
    ...
    // pass in only an options object
    if (typeof dir === 'object') { 
      options = dir;
      dir = options.attachments  
    }

    var ddoc = this
      , cwd = process.cwd()
      , twd = path.join(cwd, dir)
      , src = options.attachments ? '.' : dir;
      
    if (options.attachments) process.chdir(twd);

    var files = walk(src);

    function attach (file, options) {
      var data = fs.readFileSync(file).toString('base64');
      // if options.render (?), render the data somehow
      ddoc._attachments[file] = {
    ...
    function load (file, options) {
      ...
      // optionally render data
      if (options.operators) {
        var ext = file.split('.').pop()
          , op = options.operators[ext];
        if (op) data = op(data, options);
      }

      leaf = leaf.split('.')[0]; // make this optional and do it better
      obj[leaf] = data; 
    }

    loader = (options.attachments) ? attach : load
...
function walk (dir) {
  ...
  listings.forEach(function (listing) {
    ...
    if (stat.isFile()) {
      files.push(file); // make sure this is a relative path first
